...
also includes curated source corpora and translated corpora for the
Linux kernel tests, the Paul E. McKenney RCU tests, and the supported subset
of the RC11/C tests sourced from Dartagnan.  Running the workflow allows the reviewer to reproduce the
...
\begin{itemize}
  ...
  \item {\bf Binary: } none included, source-only Python artifact
  \item {\bf Model: } none
  \item {\bf Data set: } Linux kernel litmus tests, Paul E. McKenney RCU litmus tests, RC11/C litmus tests
  \item {\bf Run-time environment: } Linux/Unix-like command line, Python 3, local filesystem; no root access required
...
The repository uses only the Python standard library and \texttt{networkx},
which is imported by \texttt{calculus.py}. No root priveleges are needed.
...
\begin{itemize}
  ...
  \item \texttt{litmus/paulmckrcu/}: Paul E. McKenney RCU source corpus.
...
python3 scripts/run_artifact_pipeline.py \
  litmus/Kernel/MP+pooncerelease+poacquireonce.litmus --kind linux

python3 scripts/run_artifact_pipeline.py \
  litmus/c/lb.litmus --kind c
\end{verbatim}
...
\begin{enumerate}
  ...
  \item Run \texttt{scripts/run\_artifact\_pipeline.py} on the test. \\
...
python3 scripts/run_artifact_pipeline.py \
  litmus/paulmckrcu/C-WW-G+WW-R.litmus --kind linux

python3 scripts/run_artifact_pipeline.py \
  litmus/c/lb.litmus --kind c \
  --keep-translated /tmp/lb.translated.litmus
\end{verbatim}
...
\begin{itemize}
  \item \texttt{litmus/Kernel/MP+pooncerelease+poacquireonce.litmus}
  \item \texttt{litmus/paulmckrcu/C-WW-G+WW-R.litmus}
  \item \texttt{litmus/c/lb.litmus}
\end{itemize}

At the time this appendix was prepared, the condensed translated corpora
contained:
...
The unsupported cases are currently limited to atomic CAS/RMW patterns, read / pointer arithmetic (e.g. \texttt{r = *p + 1} or \texttt{t = atomic\_load\_explicit(x + 1, memory\_order\_acquire)}), and tests forbidden due to dataraces.
