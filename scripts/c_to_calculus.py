"""
Convert simple C thread-litmus tests into the
calculus_test litmus format.

Usage:
  # Convert single file -> stdout
  python3 scripts/c_to_calculus.py litmus/c/2+2W.c

  # Convert single file -> output path
  python3 scripts/c_to_calculus.py litmus/c/2+2W.c -o /tmp/2+2W.litmus
...
def convert_file_text(text: str):
    ...
    if unsupported:
        ...
        raise UnsupportedConstructError(
            "Unsupported C/RC11 constructs encountered during conversion:\n"
            f"{joined}\n"
...
def main():
    p = argparse.ArgumentParser(description="Convert C litmus tests to calculus format")
    p.add_argument('input', help='Input file (.c) or directory (for example litmus/c/)')
