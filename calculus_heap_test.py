...
from calculus_heap_parser import (
    GuardCondition,
    parse_heap_event_spec,
    parse_heap_file,
)
import os
...
def ToReachabilityRelations(edges, nodes):
    G = nx.DiGraph()
...
def ToPositiveTransitiveClosureRelations(edges, nodes):
    ...
    for node in nodes:
        ...
        while stack:
            ...
            visited.add(current)
            if current != node:
                AddUniqueRelation(closure, Relation(node, current))
...
def IsAfterSpinlock(event):
    return event.type == Operation.FENCE and event.strength == MemoryOrder.AFTER_SPINLOCK

def IsAfterUnlockLock(event):
    return event.type == Operation.FENCE and event.strength == MemoryOrder.AFTER_UNLOCK_LOCK

def IsAcquireLike(event):
    return (
        event.strength == MemoryOrder.ACQUIRE
        or event.strength == MemoryOrder.LOCK_READ
    )

def IsReleaseLike(event):
    return (
        event.strength == MemoryOrder.RELEASE
        or event.strength == MemoryOrder.UNLOCK
    )


def LockRMWPairs(threads):
    result = []

    for thread in threads:
        for index, event in enumerate(thread[:-1]):
            next_event = thread[index + 1]
            if (
                IsLockRead(event)
                and IsLockWrite(next_event)
                and EventLocation(event) == EventLocation(next_event)
            ):
                result.append((event, next_event))

    return result


def AtomicRMW(threads, rf, co):
    co_pairs = {(rel.First(), rel.Last()) for rel in co}
    rf_sources = {rel.Last(): rel.First() for rel in rf}
    writes_by_location = defaultdict(list)

    for write_event in [rel.First() for rel in co]:
        location = EventLocation(write_event)
        if location is not None and write_event not in writes_by_location[location]:
            writes_by_location[location].append(write_event)
    for write_event in [rel.Last() for rel in co]:
        location = EventLocation(write_event)
        if location is not None and write_event not in writes_by_location[location]:
            writes_by_location[location].append(write_event)

    for lock_read, lock_write in LockRMWPairs(threads):
        source_write = rf_sources.get(lock_read)
        if source_write is None:
            return False

        if not values_equal(lock_read.value, 0):
            return False

        if not values_equal(source_write.value, 0) or not values_equal(lock_write.value, 1):
            return False

        if (source_write, lock_write) not in co_pairs:
            return False

        for candidate_write in writes_by_location.get(EventLocation(lock_write), []):
            if candidate_write in {source_write, lock_write}:
                continue
            if (source_write, candidate_write) in co_pairs and (candidate_write, lock_write) in co_pairs:
                return False
...
def ToPoRel(threads):
    ...
    for thread in threads:
        for i in range(len(thread)):
            if IsReleaseLike(thread[i]) and thread[i].type != Operation.FENCE:
                for j in range(i):
            ...
            elif IsReleaseLike(thread[i]) and thread[i].type == Operation.FENCE:
                for j in range(i + 1, len(thread)):
...
def ToAcqPo(threads):
    ...
    for thread in threads:
        for i in range(len(thread)):
            if IsAcquireLike(thread[i]) and thread[i].type != Operation.FENCE:
                for j in range(i + 1, len(thread)):
                    if thread[j].type != Operation.FENCE:
                        result.append(Relation(thread[i], thread[j]))
                    else:
            ...
            elif IsAcquireLike(thread[i]) and thread[i].type == Operation.FENCE:
                for j in range(i):
                    if thread[j].type == Operation.READ or thread[j].type == Operation.RMW:
...
def ToPoUnlockLockPo(threads, rf):
    result = []

    for thread in threads:
        for unlock_index, unlock_event in enumerate(thread):
            if not IsUnlock(unlock_event):
                continue

            for lock_index in range(unlock_index + 1, len(thread)):
                lock_read = thread[lock_index]
                if not IsLockRead(lock_read):
                    continue
                for pre_index in range(unlock_index):
                    if thread[pre_index].type == Operation.FENCE:
                        continue
                    for post_index in range(lock_index + 1, len(thread)):
                        if thread[post_index].type == Operation.FENCE:
                            continue
                        result.append(
                            Relation(
                                thread[pre_index],
    ...
    for rel in rf:
        if not IsUnlock(rel.First()) or not IsLockRead(rel.Last()):
            continue

        unlock_event = rel.First()
        lock_read = rel.Last()
        unlock_thread = None
        lock_thread = None

        for thread in threads:
            if unlock_event in thread:
                unlock_thread = thread
            if lock_read in thread:
                lock_thread = thread

        if unlock_thread is None or lock_thread is None:
            continue

        unlock_index = unlock_thread.index(unlock_event)
        lock_index = lock_thread.index(lock_read)

        for pre_index in range(unlock_index):
            if unlock_thread[pre_index].type == Operation.FENCE:
                continue
            for post_index in range(lock_index + 1, len(lock_thread)):
                if lock_thread[post_index].type == Operation.FENCE:
                    continue
                result.append(
                    Relation(
                        unlock_thread[pre_index],
                        unlock_event,
                        lock_read,
                        lock_thread[post_index],
                    )
                )

    return result

def ToAfterSpinlock(threads):
    result = []

    for thread in threads:
        for fence_index, fence_event in enumerate(thread):
            if not IsAfterSpinlock(fence_event):
                continue

            lock_write_index = None
            for candidate_index in range(fence_index - 1, -1, -1):
                if IsLockWrite(thread[candidate_index]):
                    lock_write_index = candidate_index
                    break

            if lock_write_index is None:
                continue

            pre_indices = [lock_write_index]
            for pre_index in range(lock_write_index):
                if thread[pre_index].type != Operation.FENCE:
                    pre_indices.append(pre_index)

            for pre_index in pre_indices:
                pre_event = thread[pre_index]
                for post_index in range(fence_index + 1, len(thread)):
                    post_event = thread[post_index]
                    if post_event.type == Operation.FENCE:
                        continue
                    if pre_event == thread[lock_write_index]:
                        result.append(Relation(pre_event, fence_event, post_event))
                    else:
                        result.append(
                            Relation(
                                pre_event,
                                thread[lock_write_index],
                                fence_event,
                                post_event,
                            )
                        )
...
def ToAfterUnlockLock(threads, rf):
    result = []

    for thread in threads:
        for fence_index, fence_event in enumerate(thread):
            if not IsAfterUnlockLock(fence_event):
                continue

            for lock_index in range(fence_index - 1, -1, -1):
                lock_read = thread[lock_index]
                if not IsLockRead(lock_read):
                    continue

                for rel in rf:
                    if rel.Last() != lock_read or not IsUnlock(rel.First()):
                        continue

                    unlock_event = rel.First()
                    unlock_thread = None
                    for candidate_thread in threads:
                        if unlock_event in candidate_thread:
                            unlock_thread = candidate_thread
                            break

                    if unlock_thread is None:
                        continue

                    unlock_index = unlock_thread.index(unlock_event)
                    for pre_index in range(unlock_index):
                        pre_event = unlock_thread[pre_index]
                        if pre_event.type == Operation.FENCE:
                            continue
                        for post_index in range(fence_index + 1, len(thread)):
                            post_event = thread[post_index]
                            if post_event.type == Operation.FENCE:
...
def ToStrongFence(threads, rf):
    ...
    result += ToAfterSpinlock(threads)
    result += ToAfterUnlockLock(threads, rf)
    return result
...
def ToCtrl(threads, control_guards):
    result = []
    if not control_guards:
        return result

    event_by_id = {}
    reads_by_thread_register = {}

    for thread in threads:
        for event in thread:
            event_by_id[event.identifier] = event
            reg_key = RegisterKey(event.register)
            if event.type == Operation.READ and reg_key is not None:
                reads_by_thread_register[(event.identifier.getThreadId(), reg_key)] = event

    for event_id, guard in control_guards.items():
        guarded_event = event_by_id.get(event_id)
        if guarded_event is None:
            continue
        source_event = reads_by_thread_register.get((event_id.getThreadId(), guard.register))
        if source_event is None:
            continue
        result.append(Relation(source_event, guarded_event))

    return result

def ToCtrlWrite(threads, control_guards):
    return [rel for rel in ToCtrl(threads, control_guards) if rel.Last().type == Operation.WRITE]

# Incomplete. There exist more PPOs than will be computed here, but these are "sufficient" for small litmus tests
# po-rel, acq-po, strong-fence, WMB, RMB, po-unlock-lock-po
def ToPPO(threads, rf, control_guards=None):
    result = []
    ctrl_write = ToCtrlWrite(threads, control_guards or {})
    
    result += (
        ToPoRel(threads)
        + ToAcqPo(threads)
        + ToStrongFence(threads, rf)
        + ToWMB(threads)
        + ToRMB(threads)
        + ToAddr(threads)
        + ToPoUnlockLockPo(threads, rf)
        + ctrl_write
    )
...
def ToST(threads, rf):
    ...
    # rel ; rfe ; acq
    for r in rfe:
        if (IsReleaseLike(r.First()) or r.First().language == Language.LINUX) and (IsAcquireLike(r.Last()) or r.Last().language == Language.LINUX):
            result.append(r)
    ...
    for pr in po_rel:
        for r in rfe:
            if pr.Composes(r):
                ...
                if IsAcquireLike(r.Last()) or r.Last().language == Language.LINUX:
                    result.append(Relation(pr, r))
    
    # rel ; rfe ; acq-po
    for r in rfe:
        if IsReleaseLike(r.First()) or r.First().language == Language.LINUX:
            for ap in acq_po:
                if r.Composes(acq_po):
                    result.append(Relation(pr, r, ap))
...
def ToProp(threads, rf, fr, co):
    result = []
    nodes = [event for thread in threads for event in thread]
    
    synct = ToST(threads, rf)
    
    tmp = [RelationToEndpoints(rel) for rel in rf + fr + co]
    if PerLocSC(threads, rf, fr, co):
        eco = tmp.copy()
    else:
        eco = ToReachabilityRelations(
            ToEventEdges(tmp),
            nodes,
        )
        if eco is None:
    ...
    po_rel = ToPoRel(threads) + ToStrongFence(threads, rf)
    po_unlock_lock_po = ToPoUnlockLockPo(threads, rf)
    cumul_fences = po_rel.copy()
    cumul_fences += ToWMB(threads)
    cumul_fences += po_unlock_lock_po
    for r in synct:
        for pr in po_rel:
            if r.Composes(pr):
                AddUniqueRelation(cumul_fences, Relation(r, pr))

    cumul_fence_star_nonempty = ToPositiveTransitiveClosureRelations(
        ToEventEdges(cumul_fences),
        nodes,
    )
    
    result += eco + cumul_fence_star_nonempty + synct
    ...
    for e in eco:
        for cf in cumul_fence_star_nonempty:
            if e.Composes(cf):
                AddUniqueRelation(ecf, Relation(e, cf))
                
    for e in eco:
        for st in synct:
            if e.Composes(st):
                AddUniqueRelation(est, Relation(e, st))
                
    for cf in cumul_fence_star_nonempty:
        for st in synct:
            if cf.Composes(st):
                AddUniqueRelation(cfst, Relation(cf, st))
    ...
    for e in ecf:
        for st in synct:
            if e.Composes(st):
                AddUniqueRelation(result, Relation(e, st))
    
    return result

class Execution:
    def __init__(self, threads, rf, fr, co, control_guards=None):
        self.threads = threads
        self.rf = rf
        self.fr = fr
        self.co = co
        self.control_guards = control_guards or {}
        self.ctrl = ToCtrl(threads, self.control_guards)
        self.ppo = ToPPO(threads, rf, self.control_guards)
...
def ToPropagatesBeforeRelations(e):
    ...
    strong_fences = ToStrongFence(e.threads, e.rf)
...
def ToCompletePropagatesBeforeRelations(e, hb_star=None):
    ...
    result = []
    strong_fences = [RelationToEndpoints(rel) for rel in ToStrongFence(e.threads, e.rf)]
...
def convert_to_heap_events(parsed_threads):
    ...
    for thread_id, thread in enumerate(parsed_threads):
        ...
        for statement in thread:
            specs = parse_heap_event_spec(statement.line)
            if not isinstance(specs, list):
                specs = [specs]

            for spec in specs:
                identifier = iterator.next_id()
                event = Event(
                    identifier,
                    spec.location,
                    spec.type,
                    ...
                    spec.language,
                    spec.value,
                    register_from_name(identifier, spec.register_name),
                )
                thread_events.append(event)
                if statement.guard_register is not None:
                    control_guards[event.identifier] = statement.guard_register
        events.append(thread_events)

    return events, control_guards
    ...
    for thread in threads:
        for event in thread:
            if event.location_expr is not None:
                canonicalized_location = canonicalize_location_name(event.location_expr)
                if canonicalized_location is not None:
                    known_names.add(canonicalized_location)
            if isinstance(event.value, str):
                canonicalized_value = canonicalize_location_name(event.value)
                if canonicalized_value is not None:
                    known_names.add(canonicalized_value)
    ...
    for thread in template_threads:
        for event in thread:
            ...
            if isinstance(event.value, str):
                ...
                if isinstance(maybe_location, str):
                    resolved_value_location = name_to_location.get(maybe_location)
                    if resolved_value_location is not None:
                        required_locations.add(resolved_value_location)
...
def control_guard_is_true(value):
    if value is None:
        return False
...
def normalize_guard_value(value):
    ...
    return canonicalize_location_name(value)


def control_guard_matches(value, guard: GuardCondition):
    if guard.operator == "truthy":
        return control_guard_is_true(value)

    expected = normalize_guard_value(guard.value)
    if guard.operator == "==":
...
def filter_control_flow_execution(threads, rf, control_guards):
    if not control_guards:
        return threads, rf

    register_values = {}
    for thread in threads:
        thread_id = thread[0].identifier.getThreadId() if thread else None
        for event in thread:
            reg_key = RegisterKey(event.register)
            if event.type == Operation.READ and reg_key is not None:
                register_values[(event.identifier.getThreadId(), reg_key)] = event.value

    disabled_events = set()
    for event_id, guard in control_guards.items():
        guard_value = register_values.get((event_id.getThreadId(), guard.register))
        if not control_guard_matches(guard_value, guard):
            disabled_events.add(event_id)

    if not disabled_events:
        return threads, rf

    filtered_threads = []
    surviving_events = set()
    surviving_reads = set()
    for thread in threads:
        kept_events = []
        for event in thread:
            if event.identifier in disabled_events:
                continue
            kept_events.append(event)
            surviving_events.add(event)
            if event.type == Operation.READ:
                surviving_reads.add(event)
        filtered_threads.append(kept_events)

    filtered_rf = [
        rel
        for rel in rf
        if rel.First() in surviving_events and rel.Last() in surviving_events
    ]

    rf_targets = {rel.Last() for rel in filtered_rf}
    if any(read_event not in rf_targets for read_event in surviving_reads):
        return None, None

    return filtered_threads, filtered_rf
...
def resolve_location_expr(expr, registers, known_registers, name_to_location):
    ...
    if token in registers:
        ...
        if is_numeric_value(location):
            return None
        location = canonicalize_location_name(location)
        return name_to_location.get(location, location)
...
converted, control_guards = convert_to_heap_events(parsed_litmus.threads)
...
for concrete_threads, concrete_inits, rf_i in rf_executions:
    filtered_threads, filtered_rf = filter_control_flow_execution(
        concrete_threads,
        rf_i,
        control_guards,
    )
    ...
    co_candidates = enumerate_co_relations(filtered_threads, concrete_inits)
    for co_j in co_candidates:
        if not constraints_satisfied(
            ...
            continue
        fr = generate_fr_relations(filtered_rf, co_j)
        test = Execution(filtered_threads, filtered_rf, fr, co_j, control_guards)
        if (
            AtomicRMW(filtered_threads, filtered_rf, co_j)
            and
            not NoThinAir(filtered_threads, filtered_rf)
            and not PerLocSC(filtered_threads, filtered_rf, fr, co_j)
